...
def gptq_fwrd(model, dataloader, dev, args):
    ...
    for i in range(len(layers)):
        ...
        full = quant_utils.find_qlayers(layer, layers=[torch.nn.Linear, LinearBase])
...
def rtn_fwrd(model, dev, args, custom_layers=None):
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="(RtN Quant.) Layers"):
        ...
        subset = quant_utils.find_qlayers(
            layer, layers=[torch.nn.Linear, torch.nn.Embedding, LinearBase]
        )
        ...
        for name in subset:
            ...
            subset[name].weight.data.copy_(q.to(next(iter(layer.parameters())).dtype))
            if args.export_to_et:
