...
import torch
import transformers
from utils.quant_linear import QuantizeLinear
    ...
    def forward(self, x, R1=None, R2=None, transpose=False):
        ...
        if R1 is not None:
            ...
            if isinstance(x, tuple):
                x = tuple(
                    z.to(x_dtype) if isinstance(z, torch.Tensor) else z for z in x
                )
...
def add_actquant(
    ...
    layers=[
        ...
        LinearBase,  # For vLLM linear layers
    ],
) -> None:
    ...
    for attr in dir(module):
        ...
        if isinstance(tmp, tuple(layers)):
...
def find_qlayers(
    module,
    layers=[torch.nn.Linear, ActQuantWrapper, QuantizeLinear, LinearBase],
    name: str = "",
):
    # fix for llama embedding layer
    if type(module) in [torch.nn.Embedding] and isinstance(module, tuple(layers)):
